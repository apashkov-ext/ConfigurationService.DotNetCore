...
public class OptionGroupsControllerTests : ControllerTests
{
    private const string _url = "api/option-groups";
    ...
    public async void GetAll_Empty_ReturnsCorrectType()
    {
        ...
        var actual = await GetAsync<PagedResponseDto<OptionGroupDto>>(_url);
        Assert.Equal(System.Net.HttpStatusCode.OK, actual.StatusCode);
        Assert.NotNull(actual.ResponseData.Data);          
    }

    //[Fact]
    //public async void GetAll_ExistsSingle_ReturnsSingle()
    //{
    ...
    //    var expectedDto = group.ToDto();

    //    var response = await GetAsync<PagedResponseDto<OptionGroupDto>>(_url);

    //    Assert.Equal(System.Net.HttpStatusCode.OK, response.StatusCode);
    //    Assert.Single(response.ResponseData.Data);
    //    Assertions.ApplicationDtosAreEquivalentToModel(response.ResponseData.Data, group);  
    //}
    ...
    public async void GetAll_ExistsSingleWithHierarchy_ReturnsSingleWithHierarchy()
    {
        ...
        var response = await GetAsync<PagedResponseDto<ApplicationDto>>(_url);
    ...
    {
        SetupTest();
        var response = await GetAsync($"{_url}/{Guid.NewGuid()}");
    ...
    public async void GetById_Exists_ReturnsWithHierarchy()
    {
        ...
        var response = await GetAsync<ApplicationDto>($"{_url}/{project.Id}");
        Assert.Equal(System.Net.HttpStatusCode.OK, response.StatusCode);
    ...
    public async void Post_Exists_Returns422()
    {
        ...
        var response = await PostAsync(_url, body);
    ...
    public async void Post_NotExists_Returns201AndCorrectResponse()
    {
        ...
        var response = await PostAsync<CreatedApplicationDto>(_url, body);
